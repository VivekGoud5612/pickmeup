...
def boss_training():
    ...
    for i in range(4):
        ...
        while not done and rounds<50:
            ...
            obs=gamestate.get_boss_observations(0)
            mask=gamestate.get_action_mask(0)

            boss_action=agents[0].get_action(obs,mask)
            tank_action=4
            dealer_action=4
            ...
            summaries={}
            summaries[0]=ActionHandler.perform_action(0,boss_action,gamestate)
            summaries[1]=ActionHandler.perform_action(1,dealer_action,gamestate)
            summaries[2]=ActionHandler.perform_action(2,tank_action,gamestate)

            boss_step_reward=calculate_boss_sandbox_rewards(gamestate,summaries,heroes)
            ...
            agents[0].policy.store_reward(boss_step_reward,done)

            episode_rewards+=boss_step_reward
            rounds+=1
        print(f"Episode : {ep}")
        ep+=1
        agents[0].policy.learn()
        
    # agents[0].policy.save("engine/sandboxes/Boss_trained.pth")
...
def calculate_boss_sandbox_rewards(gamestate,summaries,heroes):
    reward=-0.1

    boss_stats=summaries[1].get("combat_stats") or {}
    if boss_stats.get("damage_dealt", 0) > 0:
        reward+=0.1*boss_stats.get("damage_dealt")

    for name,id in heroes.items():
        if not gamestate.is_alive(id):
            reward+=5.0
