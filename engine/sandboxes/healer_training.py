...
def healer_training():
    ...
    for i in range(4):
        ...
        while not done and rounds<50:
            ...
            healer_action=agents[0].get_action(obs,mask,)
            tank_action=4
            boss_action=get_dummy_boss_action(1,2,gamestate)
            ...
            summaries={}
            summaries[0]=ActionHandler.perform_action(0,healer_action,gamestate)
            ...
            healer_step_reward=calculate_healer_sandbox_rewards(gamestate,summaries)
            ...
            agents[0].policy.store_reward(healer_step_reward,done)

            episode_rewards+=healer_step_reward
            rounds+=1
        print(f"Episode : {ep}")
        ep+=1
        agents[0].policy.learn()
        
    agents[0].policy.save("engine/sandboxes/Healer_trained.pth")
...
def calculate_healer_sandbox_rewards(gamestate,summaries):
    reward=-0.1

    healer_stats=summaries[0].get("combat_stats") or {}
    if healer_stats.get("healed",0)>0:
         reward+=3.0

    if not gamestate.is_alive(2):
        reward-=5.0
