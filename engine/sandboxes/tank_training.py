...
def tank_training():
    ...
    for i in range(4):
        ...
        while not done and rounds<50:
            ...
            closest_enemy_for_boss=closest_enemy(1,heroes,gamestate)

            tank_action=agents[0].get_action(obs,mask,)
            dealer_action=4
            boss_action=get_dummy_boss_action(1,closest_enemy_for_boss,gamestate)
            ...
            summaries={}
            summaries[0]=ActionHandler.perform_action(0,tank_action,gamestate)
            summaries[1]=ActionHandler.perform_action(1,boss_action,gamestate)
            summaries[2]=ActionHandler.perform_action(2,dealer_action,gamestate)

            tank_step_reward=calculate_tank_sandbox_rewards(gamestate,summaries)

            # print(f"{gamestate.identities[0].role} took an action {tank_action} with reward :{tank_step_reward}")
            # print(f"{gamestate.identities[1].role} took an action {boss_action}")
            # print(f"{gamestate.identities[2].role} took an action {dealer_action}")
            if not gamestate.is_alive(0) or not gamestate.is_alive(1) or not gamestate.is_alive(2):
                done = True
            
            agents[0].policy.store_reward(tank_step_reward,done)

            episode_rewards+=tank_step_reward
            rounds+=1
        print(f"Episode : {ep}")
        ep+=1
        agents[0].policy.learn()
        
    agents[0].policy.save("engine/sandboxes/Tank_trained.pth")
...
def calculate_tank_sandbox_rewards(gamestate,summaries):
    reward=-0.1

    tank_stats=summaries[0].get("combat_stats") or {}
    if tank_stats.get("damage_dealt",0)>0:
         reward+=0.5

    boss_stats=summaries[1].get("combat_stats") or {}
    if boss_stats.get("damage_dealt",0)>0:
        target_id=boss_stats.get("target_id")

        if target_id==2:
            reward-=2.0

        elif target_id==0 and gamestate.is_blocking.get(target_id,False):
            reward+=2.0
