...
from engine.utils.enums import EngineStatus, AgentRole
...
class TrainingEngine:
    ...
    def initialize(
        ...
        run_id : UUID,   ## from ENGINE request DTO.. will be used in local engine client...
        checkpoint_directory : Path | None = None,
        checkpoint_path : Path | None = None,
        publisher : EngineEventPublisher | None = None,
    ) -> None:

        with self._lock:

            self._training_configuration = config   ## create tensor board, env, agent, rollout, workers and such..
            self._run_id = run_id  ## Coming from backend....
    ...
    def start(self) -> None:
        """
        Starts training asynchronously.

        Returns immediately after spawning the training thread.
        """
        ...
        with self._lock:
            ...
            if self._state != EngineStatus.INITIALIZED:
                raise RuntimeError(
                    "Training has not been initialized."
                )
    ...
    def save_checkpoint(self, checkpoint_path : Path, checkpoint_type : CheckpointType = CheckpointType.PERIODIC) -> None:
        ...
        self._publisher.publish(
            ...
            hyperparameters=self._training_configuration.hyperparameters,

            reward_weights=self._training_configuration.reward_weights,

            curriculum_settings=self._training_configuration.curriculum,
        )
    )
    ...
    def _training_loop(self) -> None:
        ...
        while self._state != EngineStatus.STOPPED:   # Till we pause that... 
            ...
            print(f"[CHECKPOINT] _periodic_checkpoint called, global_step={self._global_step}")
    ...
    def _collect_rollout(self) -> None:
        ...
        for _ in range(self._rollout_length):   ## Game loop is here actually... global step loop is in training loop
            ...
            actions, log_probs, values = self._magent.get_actions_and_values(
            ...
            self._global_step += (
            self._training_configuration.hyperparameters.num_envs
            )
            ...
            old_pot = np.array([info.get('old_hand', np.zeros(4)) for info in self._next_info['env_infos']])  ##  A new way to unpack all the (8,4) elements (numenvs, num_agents)... We loop over num_envs to convert that dict to an array.. If there is no dict then we just a zeros array..
            new_pot = np.array([info.get('new_hand', np.zeros(4)) for info in self._next_info['env_infos']])
            ...
            self._bootstrap_truncated_episodes_and_push_logs()
    ...
    def _bootstrap_truncated_episodes_and_push_logs(self):
        ...
        for env_idx in range(self._training_configuration.hyperparameters.num_envs):
            if self._truncated[env_idx]:  ## With the truncated information from above (which was an array converted in shared_env)
                ...
                t_action_masks = np.ones((1, 4, 8), dtype = bool)  ## A simple toy mask to make just supplement for the actual mask as we only need the values here..

                _, _, terminated_values = self._magent.get_actions_and_values(
            ...
            if 'episode_length' in env_info:  ## if the info contains these we can say that the game is indeed over..
                ...
                if 'episode_rewards' in env_info:
                    ...
                    self._rolling_effective_heal.append(np.mean(env_info['effective_heal']))
    ...
    def _update_policy(self):
        ...
        self._initialize_update_metrics()
    ...
    def _calculate_losses(self, loss_dict):
        ...
        self._avg_losses["actor"][AgentRole.TANK] += loss_dict["actor_loss"][0] ## We have per agent losses
        self._avg_losses["actor"][AgentRole.DEALER] += loss_dict['actor_loss'][1]
        self._avg_losses["actor"][AgentRole.HEALER] += loss_dict['actor_loss'][2]
        self._avg_losses["actor"][AgentRole.BOSS] += loss_dict['actor_loss'][3]

        self._avg_losses["critic"][AgentRole.TANK] += loss_dict['critic_loss'][0]
        self._avg_losses["critic"][AgentRole.DEALER] += loss_dict['critic_loss'][1]
        self._avg_losses["critic"][AgentRole.HEALER] += loss_dict['critic_loss'][2]
        self._avg_losses["critic"][AgentRole.BOSS] += loss_dict['critic_loss'][3]
    ...
    #     self._writer.add_scalar("Loss/TankActor", self._avg_losses["actor"][AgentRole.TANK], self._global_step)
    #     self._writer.add_scalar("Loss/DealerActor", self._avg_losses["actor"][AgentRole.DEALER], self._global_step)
    #     self._writer.add_scalar("Loss/HealerActor", self._avg_losses["actor"][AgentRole.HEALER], self._global_step)
    #     self._writer.add_scalar("Loss/BossActor", self._avg_losses["actor"][AgentRole.BOSS], self._global_step)

    #     self._writer.add_scalar("Loss/TankCritic", self._avg_losses["critic"][AgentRole.TANK], self._global_step)
    #     self._writer.add_scalar("Loss/DealerCritic", self._avg_losses["critic"][AgentRole.DEALER], self._global_step)
    #     self._writer.add_scalar("Loss/HealerCritic", self._avg_losses["critic"][AgentRole.HEALER], self._global_step)
    #     self._writer.add_scalar("Loss/BossCritic", self._avg_losses["critic"][AgentRole.BOSS], self._global_step)
    ...
    #     self._writer.add_scalar("Metrics/TankAdvantages", self._advantages[AgentRole.TANK], self._global_step)
    #     self._writer.add_scalar("Metrics/DealerAdvantages", self._advantages[AgentRole.DEALER], self._global_step)
    #     self._writer.add_scalar("Metrics/HealerAdvantages", self._advantages[AgentRole.HEALER], self._global_step)
    #     self._writer.add_scalar("Metrics/BossAdvantages", self._advantages[AgentRole.BOSS], self._global_step)
    ...
    def _periodic_checkpoint(self):
        ...
        save_interval = (
            self._training_configuration.hyperparameters.checkpoint_save_interval
        )

        if (
            self._global_step - self._last_checkpoint_step
            < save_interval
        ):
            return
    ...
    def _initialize_checkpoint_directory(self, checkpoint_directory : Path | None = None) -> None:
        ...
        if not checkpoint_directory:
            self._checkpoint_directory = Path(f"checkpoints/{self._run_id}")
    ...
    def _initialize_environment(self) -> None:
        ...
        env_fns = [
            partial(
                create_environment,
                grid_size=self._training_configuration.curriculum.grid_size,
                max_steps=self._training_configuration.curriculum.max_steps,
            )
            for _ in range(
                self._training_configuration.hyperparameters.num_envs
            )
        ...
        self._vector_env = SharedSubprocessVectorEnv(
            env_fns,
            num_agents=4,
        )
    ...
    def _initialize_agent(self):
        ...
        print(f"[MAIN BEFORE MAGENT] PID={os.getpid()}")
    ...
    def _initialize_buffer(self):
        ...
        num_steps = self._training_configuration.hyperparameters.rollout_length,
        num_envs = self._training_configuration.hyperparameters.num_envs,
        num_agents = 4,
        obs_shape = (ObservationBuilder.OBS_SIZE), global_state_shape = (ObservationBuilder.GLOBAL_STATE_SIZE),
        actions_shape = (8), device = self._device,
        )
    ...
    def _initialize_publisher(self, publisher : EngineEventPublisher):
