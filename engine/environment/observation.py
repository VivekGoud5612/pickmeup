...
class ObservationBuilder:
    ...
    def _extract_self_features(state : GameState, agent_id : int) -> np.ndarray:
        ...
        features[6] = stateops.is_skill_ready(state, agent_id, ActionTypes.BASIC)  #Check if basic skill ready
        features[7] = stateops.is_skill_ready(state, agent_id, ActionTypes.UTILITY)
        features[8] = stateops.is_skill_ready(state, agent_id, ActionTypes.ULTIMATE)
        ...
        if check_vision: ### A way to check if boss in in range. This is true only for agents in hero team and if the target is boss
            ...
            for teammate_id in [AgentRole.TANK, AgentRole.DEALER, AgentRole.HEALER]:
                if state.hp[teammate_id] > 0:  # For alive agents
                    ...
                    if dist_between_hero_to_boss <= ObservationBuilder.VISION_RANGE: ## If any hero agent is in range with boss, then the details are visible for all agents
                        visible = True 
                        break 
    ...
    def build_partial_obs(state : GameState, agent_id : int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        continuous_obs = np.zeros(ObservationBuilder.TOTAL_PARTIAL_DIM, dtype = np.float32)
        role_ids = np.zeros(GameState.NUM_ROLES, dtype = np.int32)   # For Role embeddings maybe.. Contains role IDs of self and other entity IDs..
    ...
    def build_full_state(state : GameState) -> Tuple[np.ndarray, np.ndarray]:  ## This is the observation builder for the global state
        ...
        global_features = np.zeros(ObservationBuilder.GLOBAL_STATE_SIZE, dtype = np.float32)  # There is no need for roles IDs here to be seperate, we just need to have a global role ID sequence according to agent index

        for idx in range(num_agents):

            feature_start_stride = idx * ObservationBuilder.TOTAL_PARTIAL_DIM   # Having 4 times the normal amount, where each agents self comes exactly once. This way we have the whole meaningful information 
            feature_end_stride = feature_start_stride + ObservationBuilder.TOTAL_PARTIAL_DIM
