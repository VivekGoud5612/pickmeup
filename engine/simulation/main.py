...
def make_env():
    return Env(grid_size = 20, max_steps = 200)  ## A simple function to instantiate the env...
...
def main():
    ...
    rolling_win_rate = deque(maxlen = 50)
    rolling_shaped_rewards = deque(maxlen = 50)  ## To see how much is handcrafted goign down....
        ...
        while global_step < TOTAL_TIMESTEPS:  # Till total time steps 
            ...
            for step in range(NUM_STEPS):
                ...
                for env_idx in range(NUM_ENVS):
                    ...
                    if dones[env_idx][0] == 1.0 or truncated[env_idx]: # Check if episode ended.. to log all the epsiode rewards and win rates
                        env_info = next_info['env_infos'][env_idx]
                        
                        if 'episode_length' in env_info:
                            rolling_episode_length.append(env_info['episode_length'])

                        if 'boss_hp' in env_info:
                            rolling_boss_hp.append(env_info['boss_hp'])

                        if 'win_rate' in env_info:
                            rolling_win_rate.append(env_info['win_rate'])
                        ...
                        if 'episode_rewards' in env_info:
                            ...
                            rolling_shaped_rewards.append(np.mean(shaped_rewards[env_idx])) ## For the last 50 games .. we need to store the shaped rewards to see how handcrafted hets degradedes as well but the shape is (8,4) so for each env we store the mean of that..
            ...
            if len(rolling_returns) > 0:
                ...
                writer.add_scalar("Environment/Win_Rate", np.mean(rolling_win_rate), global_step)
                writer.add_scalar("Environment/Shaped_Rewards", np.mean(rolling_shaped_rewards), global_step)
                writer.add_scalar("Environment/Episode_Length", np.mean(rolling_episode_length), global_step)

            if len(rolling_boss_hp) > 0:
                writer.add_scalar("Agent/Boss_HP", np.mean(rolling_boss_hp), global_step)

            if len(rolling_win_rate) == 50: ## For every 50 games we do this
                current_win_rate = np.mean(rolling_win_rate)
