...
from engine.utils.enums import ActionTypes
import numpy as np
...
class ActionHandler:
    ...
    def _apply_damage(state : GameState, attacker_id : int, target_id : int, skill_mult : float, stamina_cost : float, ignore_defence : bool = False):  # The core function which calculates and applies damage
        ...
        mitigation = 100.0 / (100.0 + base_defence)   # For combat reward calculation...
 ...
 ## IF the actual health is 10 and boss fires a 100 attack on this, then the damage would only be 10, so it helps preserve large damage attacks for other agents.    
        ...
        state.damage_reduction_by_block[target_id] = base_damage_without_block - actual_damage
    ...
    def resolve_basic_attack(state : GameState, agent_id : int):
        ...
        min_range = state.skill_min_ranges[agent_id, ActionTypes.BASIC]
        max_range = state.skill_max_ranges[agent_id, ActionTypes.BASIC]
        stamina_cost = state.skill_stamina_cost[agent_id, ActionTypes.BASIC]
    ...
    def resolve_pierce(state : GameState, agent_id : int):
        ...
        min_range = state.skill_min_ranges[agent_id, ActionTypes.UTILITY]
    ...
    def resolve_special(state : GameState, agent_id : int):
        ...
        min_range = state.skill_min_ranges[agent_id, ActionTypes.ULTIMATE]
    ...
    def resolve_aoe(state : GameState, agent_id : int):
        ...
        effective_skill_mult = skill_mult / max(1, len(enemies))   ## to divie a large amount of damage among number of enemies in range (equally for now), after we can maybe see if we can do that based on distance..
